...
xbuild /p:Configuration=Release /p:Platform=Release /target:Clean /p:OutputPath=build NControl.Mvvm.RX/NControl.Mvvm.RX.csproj
rm -rf NControl.Mvvm.RX/build/
...
xbuild /p:Configuration=Release /p:Platform=Release /target:build /p:OutputPath=build NControl.Mvvm.RX/NControl.Mvvm.RX.csproj

mkdir -p ../build
nuget pack -OutputDirectory ../build NControl.MVVM.Fluid.nuspec 
nuget pack -OutputDirectory ../build NControl.MVVM.nuspec 
nuget pack -OutputDirectory ../build NControl.MVVM.Reactive.nuspec 
